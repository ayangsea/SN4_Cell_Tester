#ifndef INC_FEB_CONSTANT_H_
#define INC_FEB_CONSTANT_H_

// **************************************** Definitions ****************************************

//TODO: Figure out these

#define FEB_CONST_CELL_MIN_VOLT_V 2.5
#define FEB_CONST_CELL_MAX_VOLT_V 4.2

#define FEB_CONST_CELL_MIN_TEMP_DC_C -20
#define FEB_CONST_CELL_MAX_TEMP_DC_C 60
//#define FEB_CONST_CELL_MIN_TEMP_CH_C 0
//#define FEB_CONST_CELL_MAX_TEMP_CH_C 45

#define FEB_CONST_MAX_CURRENT_DC_A 45
#define FEB_CONST_MAX_CURRENT_CH_A 12

#define FEB_CONST_IDLE_STATE 0
#define FEB_CONST_CHARGE_STATE 1
#define FEB_CONST_DISCHARGE_STATE 2

#define FEB_CONST_OKAY 0
#define FEB_CONST_SHUTDOWN 1

#endif /* INC_FEB_CONSTANT_H_ */
